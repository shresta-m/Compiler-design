// C program to calculate the Follow sets of a given grammar 
#include<stdio.h> 
#include<ctype.h> 
#include<string.h> 
...
{ 
	...
	{ 
		...
		donee[ptr] = ck; 
		printf(" Follow(%c) = { ", ck); 
		calc_follow[p1][p2++] = ck; 
		// Printing the Follow Sets of the grammar 
		for(i = 0 + km; i < m; i++) { 
			int temp = 0, check = 0; 
			for(temp = 0; temp < p2; temp++) 
			{ 
				if (f[i] == calc_follow[p1][temp]) 
				{ 
					check = 1; 
					break; 
				} 
